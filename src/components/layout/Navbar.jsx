...
const Navbar = () => {
  ...
  return (
    <nav data-aos="fade-down" className={`fixed top-0 z-50 w-full transition-all duration-300 ${isDarkTheme ? 'bg-white/80 backdrop-blur-lg shadow-sm py-4' : 'bg-transparent py-6 border-b border-transparent'}`}>
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link 
                key={link.name} 
                to={link.path} 
                className={`text-sm font-semibold transition-colors ${isDarkTheme ? 'text-slate-600 hover:text-emerald-600' : 'text-slate-700 hover:text-emerald-600'}`}
              >
                {link.name}
              </Link>
            ))}
